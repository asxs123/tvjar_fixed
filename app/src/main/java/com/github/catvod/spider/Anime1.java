...
public class Anime1 extends Spider {
    ...
    public String playerContent(String flag, String id, List<String> vipFlags) throws Exception {
        ...
        OkResult okResult = OkHttp.post(url, reqpayload, getHeaders2(url));
        String respbody = okResult.getBody();
        Map<String, List<String>> respHeader = okResult.getResp();
        if (respHeader.containsKey("set-cookie")) {
            List<String> cookieList = respHeader.get("set-cookie");
            for (int i = 0; i < Math.min(3, cookieList.size()); i++) {
                cookies = cookies + cookieList.get(i).split(";")[0];
                if (i < Math.min(2, cookieList.size() - 1)) {
                    cookies = cookies + ";";
                }
            }
        }
