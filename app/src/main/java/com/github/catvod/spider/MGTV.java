...
public class MGTV extends Spider {
    ...
    public String categoryContent(String str, String str2, boolean z, HashMap<String, String> hashMap) throws Exception {
        ...
        if (hashMap != null) {
            for (String str4 : hashMap.keySet()) {
                ...
                if (trim.length() != 0) {
                    str3 = str3 + "&" + str4 + "=" + URLEncoder.encode(trim, "UTF-8");
                }
            }
        }
        ...
        try {
            ...
            int total = new JSONObject(content).optJSONObject("data").optInt("totalHits", Integer.MAX_VALUE);
            JSONArray jSONArray = new JSONArray();
            int jsonlist = optJSONArray.length();
            for (int i = 0; i < jsonlist; i++) {
                JSONObject optJSONObject = optJSONArray.optJSONObject(i);
                String optString = optJSONObject.optString("title");
                String q = q(str3, optJSONObject.optString("img"));
                String optString2 = optJSONObject.optString("updateInfo");
                if (optString2.equals("")) {
                    optString2 = optJSONObject.optString("year");
                    if (optString2.equals("")) {
            ...
            jSONObject.put("pagecount", total / jsonlist);
            jSONObject.put("limit", jsonlist);
    ...
    public String detailContent(List<String> list) throws Exception {
        ...
        if (totalPage > page) {
            for (int curPage = 2; curPage <= totalPage; curPage++) {
                ...
                ArrayList<String> curArrayList = new ArrayList<>();
    ...
    public String homeContent(boolean z) throws Exception {
        ...
        if (z) {
            ...
            for (int i = 0; i < jSONArray.length(); i++) {
                String type_id = jSONArray.optJSONObject(i).optString("type_id");
                JSONArray optJSONArray2 = new JSONObject(OkHttp.string("https://pianku.api.mgtv.com/rider/config/channel/v1?channelId=" + type_id + "&platform=msite&_support=10000000&abroad=0", getHeaders("https://pianku.api.mgtv.com"))).optJSONObject("data").optJSONArray("listItems");
                JSONArray channelArr = new JSONArray();
                for (int j = 0; j < optJSONArray2.length(); j++) {
                    JSONObject jSONObject3 = new JSONObject();
                    jSONObject3.put("key", optJSONArray2.optJSONObject(j).optString("eName"));
                    jSONObject3.put("name", optJSONArray2.optJSONObject(j).optString("typeName"));
                    jSONObject3.put("value", new JSONArray());
                    JSONArray optJSONArray3 = optJSONArray2.optJSONObject(j).optJSONArray("items");
                    for (int k = 0; k < optJSONArray3.length(); k++) {
                        JSONObject jSONObject4 = new JSONObject();
                        jSONObject4.put("n", optJSONArray3.optJSONObject(k).optString("tagName"));
                        jSONObject4.put("v", optJSONArray3.optJSONObject(k).optString("tagId"));
                        jSONObject3.getJSONArray("value").put(jSONObject4);
                    }
                    channelArr.put(jSONObject3);
                }

                jSONObject5.put(type_id, channelArr);
            }
            jSONObject2.put("filters", jSONObject5);
        }
    ...
    public String searchContent(String str, boolean quick) throws Exception {
        String url = "https://mobileso.bz.mgtv.com/applet/search/v1?channelCode=mobile-wxap&q=" + str + "&pn=1&pc=10&_support=10000000&isInteract=1";
        String json = OkHttp.string(url, getHeaders(url));
        ...
        for (int i = 0; i < contents.length(); i++) {
            if (!"media".equals(contents.getJSONObject(i).optString("type"))) {
                // System.out.println(contents.getJSONObject(i).optString("type") + "  ⚠️ 请检查！！！");
                continue;
            }
            JSONObject jSONObject = contents.getJSONObject(i).getJSONArray("data").getJSONObject(0);
            // JSONArray sourceList = jSONObject.getJSONArray("sourceList");
            // String vodUrl = sourceList.getJSONObject(0).getString("url");
            String vodId = jSONObject.getString("vid");
